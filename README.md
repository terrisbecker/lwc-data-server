...
server maps it to the underlying relation. All other columns except `notes` are
required.
...
curl -X POST http://localhost:3001/api/phosphate-data \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"loc_id":"<location-uuid>","lab_id":42,"lab_case_file_number":1001,"measurement_date":"2026-07-01T00:00:00.000Z","measurement_time":"1970-01-01T09:30:00.000Z","analysis_date":"2026-07-02T00:00:00.000Z","analyte_id":"PO4","analyte_level":0.35,"unit":"mg/L"}'
...
prisma/
  ...
  migrations/                baseline (0_init) + incremental migrations (pmn photos, pmn UUIDs, uploads table)
